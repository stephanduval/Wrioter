...
# Also upload the systemd service file
echo -e "${YELLOW}📤 Uploading queue worker service file...${NC}"
...
ssh ${SSH_USER}@${SERVER_HOST} << 'ENDSSH'
    ...
    echo ""
    echo "⚠️ IMPORTANT: Queue worker setup required!"
    echo "The queue worker is needed for processing imports."
    echo "Contact your hosting provider to set up the queue worker service,"
    echo "or run the following command in a screen/tmux session:"
...
echo "3. Contact hosting provider or manually start queue worker"