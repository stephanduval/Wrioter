...
const translateMouse = computed(() => {
  if (typeof window !== 'undefined') {
    ...
    const rotateY = (window.innerWidth - 2 * x.value) / 100

    return {
      transform: `perspective(1200px) rotateX(${rotateX.value < -40 ? -20 : rotateX.value}deg) rotateY(${rotateY}deg) scale3d(1,1,1)`,
    }
  }
...
.landing-hero {
  ...
  padding-block: 9.75rem 26.875rem;
}

.hero-animation-img {
  inline-size: 90%;
  inset-block-start: -28.125rem;
